...
export class MathExpressionParser implements ExpressionParser {
  ...
  parseExpression(rawExpression: string): ParsedExpression {
    ...
    try {
      ...
      if (!normalizedExpression) {
        const error = CameraOCRErrorHandler.handleExpressionParsingError(
          rawExpression,
          new Error('Expression normalization failed')
        )
        return {
          isValid: false,
          normalizedExpression: '',
      ...
      console.log('🔍 [ExpressionParser] Syntax validation:', {
      ...
      // Final validation - try to evaluate the expression directly (avoid recursion)
      try {
        const result = this.safeEvaluate(normalizedExpression)
        if (typeof result !== 'number' || !isFinite(result)) {
          const error = CameraOCRErrorHandler.handleExpressionParsingError(
            normalizedExpression,
            new Error('Expression evaluation resulted in invalid number')
          )
          return {
            isValid: false,
            normalizedExpression,
        ...
        const error = CameraOCRErrorHandler.handleExpressionParsingError(
          normalizedExpression,
          evalError instanceof Error ? evalError : new Error('Expression evaluation failed')
        )
        return {
          isValid: false,
          normalizedExpression,
  ...
  normalizeExpression(expression: string): string {
    ...
    console.log('🔧 [ExpressionParser] Normalizing:', { input: expression })

    // Remove any non-mathematical content
    normalized = this.removeNonMathContent(normalized)
    console.log('🔧 [ExpressionParser] After removeNonMathContent:', normalized)
    ...
    normalized = normalized
      ...
      .trim()
    console.log('🔧 [ExpressionParser] After operator normalization:', normalized)
    ...
    console.log('🔧 [ExpressionParser] Final normalized:', normalized)
    return normalized
  }
  ...
  private removeNonMathContent(expression: string): string {
    ...
    const suffixPatterns = [/\s*(?:equals?|is|=).*$/i, /\s*(?:where|when|if).*$/i, /\s*[.!?].*$/]
  ...
  private validateSyntaxWithDetails(expression: string): { isValid: boolean; error?: string } {
    ...
    // Check for valid characters only (removed \s since we already cleaned spaces)
    const validCharRegex = /^[\d+\-*/().]+$/
    if (!validCharRegex.test(cleaned)) {
      return {
        isValid: false,
        error: `Expression contains invalid characters. Use only numbers and +, -, *, /, (, ). Got: "${cleaned}"`
      }
    }
    ...
    if (!this.hasValidOperatorPlacement(cleaned)) {
      return {
        isValid: false,
        error: 'Invalid operator placement. Operators cannot be at the start or end of expression'
      }
    }
