...
// Ref to calculator display for focus management (Requirement 4.4)
const calculatorDisplayRef = ref<InstanceType<typeof CalculatorDisplay> | null>(null);
 ...
 * Requirements: 4.1, 4.2, 4.4 - Auto-insert expression, close modal, focus calculator
 */
const handleCameraResult = async (expression: string) => {
  try {
    ...
    // Requirement 4.1, 4.4: Close camera modal automatically after successful recognition
    closeCamera();
    
    // Requirement 4.4: Focus calculator display for user interaction
    setTimeout(() => {
      calculatorDisplayRef.value?.focus();
    }, 100);
